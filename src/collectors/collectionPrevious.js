module.exports = {
	name: "collectionPreviousCollector",
	async execute(interaction) {
		const filter = i => i.customId === "collectionPrevious" && i.user.id === interaction.user.id;;
		...
		collector.on("collect", async i => {
			...
			const res = await interaction.client.db.mirors.get("UserSeries").toMessage(interaction.client.db.mirors, {
				page: parseInt(i.message.embeds[0].footer.text.slice(5).split("/")) - 2,
